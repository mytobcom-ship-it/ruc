...
bool CGISUtil::SgmtMatch(SGMT_MATCH_INPUT& stSgmtMatchInput, SGMT_INFO& stSgmtInfo, SGMT_MATCH_RES *pstSgmtMatchRes)
{
	...
	const double dfSegLenCoord = stSgmtInfo.dfLen / 360000.0;
	POINT stIntersect;

	if ((dfIntersectSgmtDistance <= dfSegLenCoord) && (dfIntersectSgmtDistance >= 0))
	{
	...
	else if (dfIntersectSgmtDistance > dfSegLenCoord)
	{
