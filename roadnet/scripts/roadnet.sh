...
do_import() {
    ...
    PY_ARGS=(--dbname "$PGDATABASE" --user "$PGUSER" --data-dir "$ROOT/data" --turninfo "$ROOT/TURNINFO.dbf")
    ...
    if command -v ogr2ogr >/dev/null 2>&1 && [ -z "${FORCE_PYTHON:-}" ]; then
        ...
        ogr2ogr -f PostgreSQL "$OGR_CONN" "$ROOT/data/MOCT_NODE.shp" \
            -nln network.moct_node -lco GEOMETRY_NAME=geom -lco SPATIAL_INDEX=GIST -lco FID= \
            -nlt PROMOTE_TO_MULTI -s_srs EPSG:5179 -t_srs EPSG:5179 -overwrite
        ogr2ogr -f PostgreSQL "$OGR_CONN" "$ROOT/data/MOCT_LINK.shp" \
