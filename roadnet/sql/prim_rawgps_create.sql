...
CREATE TABLE ROADNET.PRIM_RAWGPS (
    -- PK (운행 자연키)
...
COMMENT ON COLUMN ROADNET.PRIM_RAWGPS.REVERSE_FIT IS
    'TRUE=세그먼트 역방향(T→F)이 정방향보다 더 잘 맞아 채택됨(역주행 의심 신호, 단정 아님). 링크 자체의 방향성 데이터가 없어 사후 분석용 참고 신호. MATCHED 시에만 유효, 그 외 NULL (2026-07-18 최정우 추가)';
